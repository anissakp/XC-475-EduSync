...
const getActualToken = async (code:any) => {
  try {
    ...
    const response = await fetch(
      ...
      `https://ec2-3-87-255-160.compute-1.amazonaws.com/learn/api/public/v1/oauth2/token?code=${code}&redirect_uri=http://localhost:5174/dashboard`,
      {
        body: requestBody,
        method: "POST",
        headers: {
...
export const getConnection = onRequest({cors: true}, async (req, res)=>{
  try {
    ...
    const response = await fetch(
      ...
      `https://ec2-3-87-255-160.compute-1.amazonaws.com/learn/api/public/v1/oauth2/authorizationcode?redirect_uri=http://localhost:5174/dashboard&response_type=code&client_id=${process.env.CLIENT_ID}&scope=read&state=DC1067EE-63B9-40FE-A0AD-B9AC069BF4B0`,
      {
        headers: {
